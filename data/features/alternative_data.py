...
class AlternativeDataConfig:
    ...
    def __post_init__(self):
        ...
        if self.macro_indicators is None:
            self.macro_indicators = ['vix', 'dxy', 'yield_10y', 'oil_price', 'gold_price']
        if self.social_platforms is None:
            self.social_platforms = ['twitter', 'reddit', 'stocktwits']
        if self.fundamental_metrics is None:
            self.fundamental_metrics = ['pe_ratio', 'pb_ratio', 'debt_equity', 'roe', 'roa']
...
class SentimentFeatures:
    ...
    def _initialize_sentiment_lexicons(self):
        ...
        self.positive_words = {
            ...
            'positive', 'good', 'excellent', 'outperform', 'beat', 'exceed',
            'momentum', 'rally', 'surge', 'boom', 'bull', 'upgrade'
        }
        
        self.negative_words = {
            'bearish', 'sell', 'weak', 'decline', 'loss', 'fall', 'down',
            'negative', 'bad', 'poor', 'underperform', 'miss', 'disappoint',
            'crash', 'drop', 'plunge', 'bear', 'downgrade', 'risk'
        }
        
        self.uncertainty_words = {
            'uncertain', 'volatile', 'risk', 'concern', 'worry', 'fear',
            'doubt', 'question', 'unclear', 'unknown', 'maybe', 'might'
        }
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in text_columns:
            if col in data.columns:
                ...
                for window in self.config.sentiment_windows:
                    features[f'{col}_sentiment_ma_{window}'] = sentiment_scores.rolling(window).mean()
                    features[f'{col}_sentiment_std_{window}'] = sentiment_scores.rolling(window).std()
        ...
        for col in sentiment_columns:
            if col in data.columns:
                ...
                # Sentiment extremes
                features[f'{col}_extreme_positive'] = (sentiment > sentiment.quantile(0.9)).astype(int)
                features[f'{col}_extreme_negative'] = (sentiment < sentiment.quantile(0.1)).astype(int)
        ...
        if 'news_count' in data.columns:
            ...
            for window in self.config.sentiment_windows:
                ...
                features[f'news_attention_{window}'] = (news_count / 
                                                       news_count.rolling(window*4).mean()).fillna(1)
        ...
        if len(sentiment_cols) >= 2:
            ...
            for i, col1 in enumerate(sentiment_cols):
                for col2 in sentiment_cols[i+1:]:
                    corr_name = f'sentiment_corr_{col1.split("_")[0]}_{col2.split("_")[0]}'
                    features[corr_name] = features[col1].rolling(30).corr(features[col2])
                    
                    div_name = f'sentiment_div_{col1.split("_")[0]}_{col2.split("_")[0]}'
    ...
    def _calculate_text_sentiment(self, text: str) -> float:
        ...
        sentiment = (positive_count - negative_count - 0.5 * uncertainty_count) / total_words
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'news_embeddings' in data.columns:
            ...
            for window in [3, 7, 14]:
                ...
                avg_embeddings = embeddings.rolling(window).apply(
                    ...
                    if any(emb is not None for emb in x) else np.zeros(self.config.news_embedding_dim),
                    raw=False
                )
                
                # Embedding similarity to recent average
                for i in range(min(10, self.config.news_embedding_dim)):  # Use first 10 dimensions
                    features[f'embedding_avg_{window}_dim_{i}'] = avg_embeddings.apply(
                        lambda x: x[i] if isinstance(x, np.ndarray) and len(x) > i else 0
                    )
        ...
        if 'news_text' in data.columns:
            for topic, keywords in self.topic_keywords.items():
                ...
                # Topic momentum
                for window in [3, 7]:
                    features[f'topic_{topic}_ma_{window}'] = topic_scores.rolling(window).mean()
        ...
        if 'news_topics' in data.columns:
            ...
            for topic in unique_topics[:20]:  # Limit to top 20 topics
                ...
                for window in [7, 14]:
                    features[f'topic_freq_{topic}_{window}'] = topic_indicator.rolling(window).sum()
        ...
        if 'news_source' in data.columns:
            ...
            features['news_source_credible'] = data['news_source'].apply(
                lambda x: 1 if isinstance(x, str) and x.lower() in credible_sources else 0
            )
...
class MacroeconomicFeatures:
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for indicator in self.config.macro_indicators:
            if indicator in data.columns:
                ...
                for window in [5, 20, 50]:
                    ...
                    features[f'macro_{indicator}_deviation_{window}'] = (values - ma) / ma
                
                # Volatility
                for window in [10, 20]:
                    features[f'macro_{indicator}_vol_{window}'] = values.pct_change().rolling(window).std()
                
                # Percentile ranks (regime indicators)
                for window in [50, 100, 252]:
                    features[f'macro_{indicator}_percentile_{window}'] = (
                        values.rolling(window).rank(pct=True)
                    )
        
        # Cross-indicator relationships
        available_indicators = [ind for ind in self.config.macro_indicators if ind in data.columns]
        ...
        if 'vix' in data.columns and 'dxy' in data.columns:
            ...
            vix_norm = (data['vix'] - data['vix'].rolling(252).mean()) / data['vix'].rolling(252).std()
            dxy_norm = (data['dxy'] - data['dxy'].rolling(252).mean()) / data['dxy'].rolling(252).std()
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for platform in self.config.social_platforms:
            ...
            if sentiment_col in data.columns:
                ...
                for window in [1, 3, 7]:
                    features[f'social_{platform}_sentiment_ma_{window}'] = sentiment.rolling(window).mean()
                    features[f'social_{platform}_sentiment_std_{window}'] = sentiment.rolling(window).std()
            ...
            for metric in volume_metrics:
                ...
                if col in data.columns:
                    ...
                    # Rolling statistics
                    for window in [3, 7, 14]:
                        features[f'social_{platform}_{metric}_ma_{window}'] = values.rolling(window).mean()
                        features[f'social_{platform}_{metric}_sum_{window}'] = values.rolling(window).sum()
            
            # Engagement ratios
            if f'{platform}_likes' in data.columns and f'{platform}_posts' in data.columns:
        ...
        sentiment_platforms = [p for p in self.config.social_platforms 
                             if f'{p}_sentiment' in data.columns]
        
        if len(sentiment_platforms) >= 2:
            # Sentiment consensus
            sentiment_values = [data[f'{p}_sentiment'].fillna(0) for p in sentiment_platforms]
            ...
            for i, p1 in enumerate(sentiment_platforms):
                for p2 in sentiment_platforms[i+1:]:
                    corr = data[f'{p1}_sentiment'].rolling(30).corr(data[f'{p2}_sentiment'])
        ...
        for platform in self.config.social_platforms:
            if f'{platform}_shares' in data.columns:
                ...
                features[f'social_{platform}_viral'] = (shares > share_threshold).astype(int)
        ...
        for platform in self.config.social_platforms:
            if f'{platform}_mentions' in data.columns:
                ...
                features[f'social_{platform}_mention_acceleration'] = mention_change.diff()
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for metric in self.config.fundamental_metrics:
            if metric in data.columns:
                ...
                # Changes and trends
                features[f'fundamental_{metric}_change_1q'] = values.diff(63)  # ~1 quarter
                features[f'fundamental_{metric}_change_1y'] = values.diff(252)  # ~1 year
        ...
        if 'earnings_date' in data.columns:
            ...
            features['earnings_proximity'] = np.exp(-abs(days_to_earnings) / 30)  # Decay function
        
        if 'earnings_surprise' in data.columns:
            surprise = data['earnings_surprise'].fillna(0)
            features['earnings_surprise'] = surprise
            features['earnings_surprise_ma_4q'] = surprise.rolling(4).mean()  # 4 quarters
        ...
        if len(available_valuations) >= 2:
            ...
            for metric in available_valuations:
                ...
                features[f'fundamental_{metric}_zscore'] = (values - rolling_mean) / (rolling_std + 1e-8)
        ...
        for metric in growth_metrics:
            if metric in data.columns:
                ...
                features[f'fundamental_{metric}_consistency'] = (
                    (growth > 0).rolling(4).sum() / 4  # Fraction of positive growth quarters
                )
...
class AlternativeDataEngine:
    ...
    def calculate_all_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature_type, calculator in self.feature_calculators.items():
            try:
                ...
                features.columns = [f"alt_{feature_type}_{col}" for col in features.columns]
    ...
    def calculate_subset(self, data: pd.DataFrame, 
        ...
        for feature_type in feature_types:
            if feature_type in self.feature_calculators:
                ...
                calc_features.columns = [f"alt_{feature_type}_{col}" 
    ...
    def get_data_requirements(self) -> Dict[str, List[str]]:
        ...
        return {
            'sentiment': ['news_text', 'social_text', 'news_sentiment', 'social_sentiment'],
            'news_embedding': ['news_embeddings', 'news_text', 'news_topics', 'news_timestamp'],
            'macro': self.config.macro_indicators,
            'social': [f'{p}_{m}' for p in self.config.social_platforms 
                      for m in ['sentiment', 'mentions', 'posts', 'likes']],
            'fundamental': self.config.fundamental_metrics + ['earnings_date', 'earnings_surprise']
        }